resource "azurerm_mssql_server_security_alert_policy" "sql_server" {
  for_each = var.sql_server_security_alerting_enabled ? toset(["enabled"]) : []
...
resource "azurerm_mssql_server_vulnerability_assessment" "sql_server" {
  for_each = var.sql_server_vulnerability_assessment_enabled ? toset(["enabled"]) : []
...
resource "azurerm_mssql_server_extended_auditing_policy" "sql_server" {
  for_each = var.sql_server_extended_auditing_enabled ? toset(["enabled"]) : []
...
resource "azurerm_mssql_database_extended_auditing_policy" "elastic_pool_db" {
  for_each = var.databases_extended_auditing_enabled ? { for db in var.databases : db.name => db if var.elastic_pool_enabled == true } : {}
...
resource "azurerm_mssql_database_extended_auditing_policy" "single_db" {
  for_each = var.databases_extended_auditing_enabled ? { for db in var.databases : db.name => db if var.elastic_pool_enabled == false } : {}
